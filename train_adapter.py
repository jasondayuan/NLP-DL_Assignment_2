...
def main():
    ...
    # Parse Arguments
    parser = HfArgumentParser((ModelArguments, DataArguments, TrainingArguments, adapters.AdapterArguments))
    ...
    # Initialize wandb
    wandb.init(project=f"Task3-{model_args.model_name_or_path}-{data_args.dataset_name}", 
    ...
    model = adapters.AutoAdapterModel.from_pretrained(
        model_args.model_name_or_path,
        config=config,
    )
    model.add_classification_head(data_args.dataset_name, num_labels=num_labels)
